...
class Agent_historyController extends Controller {
	 ...
	 public function index(Agent_history $agent_history , Request $request)
	{
	 $agent_histories = $agent_history
		 ...
		 ->select(array('agent_histories.id as agent_historyID','agent.name as agent_name', 'player.name as player_name',
		 'agent_histories.from_date as from_date','agent_histories.to_date as to_date'))
