			  ...
			  <div class="row">
				    <div class="col-md-12">
						...
						<a class="btn btn-primary add" data-toggle="modal" data-target="#addModal" style="margin-bottom:20px;" >
							<i class="fa fa-plus-circle"  style="font-size: 18px;"></i> أضافة جديد
												</a>
						<div class="widget-content-white glossed">
								<div class="padded">
										<table id="matchs" class="table table-striped table-bordered table-hover datatable">
													<thead>
															<tr>
																	<th class="col-md-3">الرؤية</th>
                                  <th class="col-md-2">الرسالة</th>
				...
				<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true" data-backdrop="static" data-keyboard="false">
    				<div class="modal-dialog">
        				<div class="modal-content">
                            ...
                            <form role="form" method="POST" class="addForm" action="{{ url('/about/store') }}" data-toggle="validator">
        ...
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i>اضافة جديد</h4>
                    ...
                    <form role="form" method="POST" class="editForm" action="{{ url('/about/update') }}" data-toggle="validator">
                        <div class="modal-body">
                                @include('about.form')
                        </div>
                      <div class="modal-footer">
...
<script src="http://malsup.github.com/jquery.form.js"></script>
  ...
  var self = $(this);
       self.button('loading');
       $.ajax({
           url: "{{ url('about') }}" + "/" + self.data('id') + "/edit" ,
           type: "GET",
   ...
   $(document.body).validator().on('click', '.publish', function() {
       ...
       self.button('loading');
       $.ajax({
           url: "{{ url('about') }}" + "/" + self.data('id') + "/publish" ,
           type: "GET",
           success: function(res){
               self.button('reset');
               $('.alerts-list').append(
   ...
   $(document.body).validator().on('click', '.nopublish', function() {
       ...
       self.button('loading');
       $.ajax({
           url: "{{ url('about') }}" + "/" + self.data('id') + "/nopublish" ,
           type: "GET",
           success: function(res){
               self.button('reset');
               $('.alerts-list').append(
