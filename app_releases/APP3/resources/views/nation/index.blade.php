...
<ul class="alerts-list" style="display:none;" id="show">
  <li>
     <div class="alert alert-success alert-dismissable">
           <i class="icon-remove-sign"></i> nation has been successfully added!.
...
<ul class="alerts-list" style="display:none;" id="showupdate">
  <li>
     <div class="alert alert-success alert-dismissable">
           <i class="icon-remove-sign"></i> nation has been successfully updated!.
           <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
       </div>
   </li>
</ul>
<a class="btn btn-primary" data-toggle="modal" data-target="#addModal" style="margin-bottom:20px;" >
		<i class="fa fa-plus-circle"  style="font-size: 18px;"></i> Add nation
</a>
<div class="widget-content-white glossed">
		<div class="padded">
				<table id="nations" class="table table-striped table-bordered table-hover datatable">
						<thead>
								<tr>

										<th class="col-md-1">Country name</th>
										<th class="col-md-1">Nickname</th>
										<th class="col-md-1">Continent</th>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i> Add nation</h4>
            </div>
            <form role="form" method="POST" class="addForm" action="{{ url('/nation/store') }}" data-toggle="validator" enctype="multipart/form-data">
...
<div class="modal fade" id="editnationModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            ...
            <form role="form" id="update_form" method="POST" class="editForm" data-id="" action="{{ url('/nation/update') }}" data-toggle="validator"enctype="multipart/form-data">
                <div class="modal-body">
                    @include('nation.form')
                </div>
                <div class="modal-footer">
    ...
    $('#datePicker2')
        ...
        .on('changeDate', function(e) {
            // Revalidate the date field
            $('.addForm').formValidation('revalidateField', 'date2');
        });
	...
	oTable.draw();
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('nation') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
	                 $data = JSON.parse(res.data);
									 populateForm($data, document.getElementsByClassName("editForm")[0] );
	                 $('#editnationModal form').attr("data-id", self.data('id') );
