...
<ul class="alerts-list" style="display:none;" id="show">
  <li>
     <div class="alert alert-success alert-dismissable">
           <i class="icon-remove-sign"></i> nation cloth has been successfully added!.
...
<ul class="alerts-list" style="display:none;" id="showupdate">
  <li>
     <div class="alert alert-success alert-dismissable">
           <i class="icon-remove-sign"></i> nation cloth has been successfully updated!.
           <button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
       </div>
   </li>
</ul>
<a class="btn btn-primary" data-toggle="modal" data-target="#addModal" style="margin-bottom:20px;" >
		<i class="fa fa-plus-circle"  style="font-size: 18px;"></i> Add nation cloths
</a>
<div class="widget-content-white glossed">
		<div class="padded">
				<table id="Tcloths" class="table table-striped table-bordered table-hover datatable">
						<thead>
								<tr>
										<th class="col-md-2">Nation</th>
										<th class="col-md-2">Principle Cloths</th>
										<th class="col-md-2">Reserve Cloths</th>
										<th class="col-md-2">Reserve2 Cloths</th>
										<th class="col-md-2">Practise Cloths</th>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i> Add nationcloth</h4>
            </div>
            <form role="form" method="POST" class="addForm" action="{{ url('/nationcloth/store') }}" data-toggle="validator" enctype="multipart/form-data">
                <div class="modal-body">
                    @include('nationcloth.form')
                </div>
...
<div class="modal fade" id="editNclothModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            ...
            <form role="form" id="update_form" method="POST" class="editForm" data-id="" action="{{ url('/nationcloth/update') }}" data-toggle="validator"enctype="multipart/form-data">
                <div class="modal-body">
                    @include('nationcloth.form')
                </div>
                <div class="modal-footer">
    ...
    oTable.draw();
	  ...
	  oTable.draw();
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('nationcloth') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
	                 $data = JSON.parse(res.data);
	                 populateForm($data, document.getElementsByClassName("editForm")[0] );
	                 $('#editNclothModal form').attr("data-id", self.data('id') );
	                 $('#editNclothModal').modal('show');
	             },
	             error: function(){
	                 self.button('reset');
	                 $('.alerts-list').append(
	                     '<li>\
							...
							oTable = $('#Tcloths').DataTable({
								...
								"columns": [
										{data: 'nationname', name: 'nationname'},
