...
<a class="btn btn-primary" data-toggle="modal" data-target="#addModal" style="margin-bottom:20px;" >
		<i class="fa fa-plus-circle"  style="font-size: 18px;"></i> Add offside
</a>
...
<div class="modal fade" id="addModal" tabindex="-1" role="dialog" aria-labelledby="addModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form role="form" method="POST" class="addForm" action="{{ url('/offside/store') }}" data-toggle="validator">
                <div class="modal-body">
                    @include('offside.form')
                </div>
...
<div class="modal fade" id="editcityModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form role="form" id="update_form" method="POST" class="editForm" data-id="" action="{{ url('/offside/update') }}" data-toggle="validator">
                <div class="modal-body">
                    @include('offside.form')
                </div>
                <div class="modal-footer">
	...
	$(document).ready(function() {
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('offside') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
	                 ...
	                 $('.alerts-list').append(
		 		     ...
		 		     $("#editcityModal form").validator().on('submit', function(e){
		 		         if (!e.isDefaultPrevented())
		 		         {
		 		             var self = $(this);
		 		             $.ajax({
		 		                 url: "{{ url('offside') }}" + "/" +  self.attr("data-id"),
		 		                 type: "POST",
