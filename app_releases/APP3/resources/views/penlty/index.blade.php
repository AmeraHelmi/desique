...
<a class="btn btn-primary" data-toggle="modal" data-target="#addModal" style="margin-bottom:20px;" >
		<i class="fa fa-plus-circle"  style="font-size: 18px;"></i> Add penlty
</a>
						...
						<thead>
								<tr>
				  					<th class="col-md-2">Player</th>
										...
										<th class="col-md-2">Corner Side</th>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="addModalLabel"><i class="fa fa-plus-circle"></i> Add penlty</h4>
            </div>
            <form role="form" method="POST" class="addForm" action="{{ url('/penlty/store') }}" data-toggle="validator">
...
<div class="modal fade" id="editcityModal" tabindex="-1" role="dialog" aria-labelledby="editEmployeeModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form role="form" id="update_form" method="POST" class="editForm" data-id="" action="{{ url('/penlty/update') }}" data-toggle="validator">
                <div class="modal-body">
                    @include('penlty.form')
                </div>
                <div class="modal-footer">
	...
	$(document).ready(function() {
	    ...
	    var self = $(this);
	         self.button('loading');
	         $.ajax({
	             url: "{{ url('penlty') }}" + "/" + self.data('id') + "/edit" ,
	             type: "GET",
	             success: function(res){
	                 self.button('reset');
	                 ...
	                 $('.alerts-list').append(
		 		     ...
		 		     $("#editcityModal form").validator().on('submit', function(e){
		 		         if (!e.isDefaultPrevented())
		 		         {
		 		             var self = $(this);
		 		             $.ajax({
		 		                 url: "{{ url('penlty') }}" + "/" +  self.attr("data-id"),
		 		                 type: "POST",
							...
							oTable = $('#cities').DataTable({
								...
								"columns": [
		  							{data: 'player_id', name: 'Player_id'},
										...
										{data: 'corner_side', name: 'corner_side'},
										{data: 'time', name: 'time'},
